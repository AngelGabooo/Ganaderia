...
const Header = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (isMenuOpen) return; // No hacer scroll con menú abierto
  ...
  >
    ...
    >
      ...
      {/* Menú móvil como overlay separado */}
      <div className={`
        fixed inset-0 z-40 transition-all duration-300
        ${isMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}
        md:hidden
        bg-amber-800/95 backdrop-blur-sm
        pt-20 /* Deja espacio para el header */
      `}>
        <div className="relative h-full w-full overflow-y-auto">
          <nav className="flex flex-col items-center justify-start h-full pb-10 px-6 space-y-6">
            <NavItem
              href="/"
              mobile
              onClick={closeMenu}
